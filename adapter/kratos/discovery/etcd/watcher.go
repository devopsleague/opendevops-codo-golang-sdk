...
func newWatcher(ctx context.Context, log *log.Helper, key, name string, client *clientv3.Client) (*watcher, error) {
	w := &watcher{
		log:         log,
		key:         key,
		client:      client,
...
func (w *watcher) getInstance() ([]*registry.ServiceInstance, error) {
	...
	for _, kv := range resp.Kvs {
		...
		if err != nil {
			w.log.Errorf("unmarshal value failed: %v, key=%s", err, kv.Key)
			continue
		}
